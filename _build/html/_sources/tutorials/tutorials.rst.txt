
Tutorials
=========



.. |cat| image:: /_static/tutorial_pics/catalog.png
   :align: middle
   :target: get_catalog.html

.. |rm| image:: /_static/tutorial_pics/regional_map.png
   :align: middle
   :target: regional_map_gridded.html

.. |sm| image:: /_static/tutorial_pics/sparse_mapping.png
  :align: middle
  :target: regional_map_sparse.html

.. |ts| image:: /_static/tutorial_pics/TS.png
   :align: middle
   :target: time_series.html

.. |dep| image:: /_static/tutorial_pics/depth_profile.png
   :align: middle
   :target: depth_profile.html

.. |hst| image:: /_static/tutorial_pics/hist.png
  :align: middle
  :target: histogram.html


.. |sp| image:: /_static/tutorial_pics/section.png
   :align: middle
   :target: section.html

.. |esv| image:: /_static/tutorial_pics/esv.png
  :align: middle
  :target: ESV.html

.. |xy| image:: /_static/tutorial_pics/xy.png
  :align: middle
  :target: XY.html

.. |ct| image:: /_static/tutorial_pics/sailboat.png
  :scale: 30%
  :class: align-center
  :target: cruise_track.html



.. |lag| image:: /_static/tutorial_pics/lagrangian.png
   :align: middle
   :target: lagrangian.html

.. |ed| image:: /_static/tutorial_pics/eddy.png
   :align: middle
   :target: eddy.html

.. |fr| image:: /_static/tutorial_pics/front.png
   :align: middle
   :target: front.html

.. |cd| image:: /_static/tutorial_pics/colocalize.png
  :align: middle
  :target: custom_data.html


.. |rd| image:: /_static/tutorial_pics/subset.png
   :align: middle
   :target: retrieve_data.html

.. |sql| image:: /_static/tutorial_pics/sql_2.png
  :align: middle
  :target: sql.html

.. |cp| image:: /_static/tutorial_pics/cruise2.png
  :align: middle
  :scale: 30%
  :target: cruise_planning.html




The tutorials include both GUI and script approaches. The GUI tutorials may include a video demonstration and the script approach include example code.


+---------------------------+---------------------------+---------------------------+
| |cat|                     |    |rm|                   |           |sm|            |
+---------------------------+---------------------------+---------------------------+
|     **Get Catalog**       | **Regional Gridded Map**  |  **Regional Sparse Map**  |
+---------------------------+---------------------------+---------------------------+
| |sp|                      |    |ts|                   |          |dep|            |
+---------------------------+---------------------------+---------------------------+
|**Section Plot**           |  **Time Series**          |       **Depth Profile**   |
+---------------------------+---------------------------+---------------------------+
|  |xy|                     |    |hst|                  |            |esv|          |
+---------------------------+---------------------------+---------------------------+
|  **Plot XY**              |      **Histogram**        |     **ESV Track**         |
+---------------------------+---------------------------+---------------------------+
| |ct|                      |    |lag|                  |            |ed|           |
+---------------------------+---------------------------+---------------------------+
|**Cruise Track**           |  **Lagrangian Sampling**  |  **Eddy Sampling**        |
+---------------------------+---------------------------+---------------------------+
| |fr|                      |    |cd|                   |            |rd|           |
+---------------------------+---------------------------+---------------------------+
|  **Front Sampling**       |**Custom Data**            |**Retrieve Dataset Subset**|
+---------------------------+---------------------------+---------------------------+
| |sql|                     |        |cp|               |                           |
+---------------------------+---------------------------+---------------------------+
| **SQL Queries**           | **Cruise Planning**       |                           |
+---------------------------+---------------------------+---------------------------+




.. toctree::
  :maxdepth: 1
  :hidden:

  get_catalog
  regional_map_gridded
  regional_map_sparse
  section
  time_series
  depth_profile
  XY
  histogram
  ESV
  section
  cruise_track
  lagrangian
  eddy
  front
  custom_data
  retrieve_data
  sql
  cruise_planning
